...
class MessageProcessor(threading.Thread):
    ...
    def process_client_message(self, message, client):
        ...
        # При запросе публичного ключа пользователя
            ...
            if response[DATA]:
                ...
                response = RESPONSE_400
                response[ERROR] = 'Для данного пользователя нет публичного ключа'
                try:
                    send_message(client, response)
                except OSError:
                    self.remove_client(client)
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        ...
                        logger.debug(f'Получены данне о клиентском исключени.', exc_info=err)
    ...
    def authorize_user(self, message, sock):
        ...
        # Зарегистрирован ли пользователь на сервере
            ...
            try:
                logger.debug(f'Неизвестное имя пользователя, отправка {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
